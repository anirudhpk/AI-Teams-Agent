...
from graph_sender import fetch_message, send_message, fetch_unread_emails, get_token
...
def extract_sharepoint_link(text):
    m = re.search(r"https://callaider\.sharepoint\.com/\S+", text)
...
async def webhook(req: Request):
    ...
    for n in data.get("value", []):
        ...
        lower = text.lower()   # ✅ FIX: always defined
        ...
        if sender_id == OWNER_ID and lower == "summary":
            ...
            reply = (
                "📌 <b>Emails requiring your action</b><br><br>"
            )
            ...
            reply += (
                "<br>📨 <b>Other unread emails (FYI)</b><br><br>"
            )
        ...
        # -------------------------------------------------
        if sender_id == OWNER_ID and lower.startswith("approve access"):
            links = extract_links(body)

            file_url = links[0]  # first SharePoint link
            parts = text.split()
            permission = parts[2]  # view / edit
            
            item = sharepoint_url_to_drive_item(OWNER_ID,file_url)

            with get_conn() as c:
                c.execute(
                    "INSERT OR REPLACE INTO preapproved_files VALUES (?,?,?)",
                    (OWNER_ID, item["id"], permission)
                )
                c.commit()
            ...
            send_message(
                chat_id,
                OWNER_ID,
                f"✅ Access approved ({permission}) for:\n{file_url}"
            )
            continue    

        # -------------------------------------------------
        # ACCESS APPROVAL - NON-OWNER 
        # -------------------------------------------------     
        if sender_id != OWNER_ID:
            links = extract_links(body)
            ...
            if file_url:
                ...
                if row:
                    ...
                    send_message(
                        chat_id=chat_id,
                        user_id=OWNER_ID,  # still Anirudh's token
                        text=(
                            "🤖 **Anirudh’s AI assistant**\n\n"
                    ...
                    send_message(
                        chat_id,
                        OWNER_ID,
                        "❌ No pre-approval found for this file."
                    )
        continue
        # -------------------------------------------------
        # AUTO REPLY (NON-OWNER ONLY)
        # -------------------------------------------------
